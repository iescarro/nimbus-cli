...
class File:
    ...
    def create_backup(self, client, folder_path, db_password, db_name):
        ...
        final_zip = f"/tmp/{os.path.basename(folder_path.rstrip('/'))}.zip"
        
        # Get the folder name to zip (preserves structure)
        folder_name = os.path.basename(folder_path.rstrip('/'))
        parent_dir = os.path.dirname(folder_path.rstrip('/'))
        
        zip_cmd = f"""
        cd {parent_dir} && \
        zip -r {final_zip} {folder_name} && \
        echo "Backup created: {final_zip}"
        """
        # print(f"📤 Folder backup command: zip -r {final_zip} {folder_path}")
        print(f"Running command: {zip_cmd}")
        return run_ssh_command(client, zip_cmd)

    def transfer_from_source(self, source_client, remote_path, local_path):
        print("📥 Downloading backup file to local machine...")
        with SCPClient(source_client.get_transport()) as scp:
            scp.get(remote_path, local_path)

    def transfer_to_dest(self, dest_client, local_path, remote_path):
    ...
    def backup(source):
        ...
        # Parse source and target
        src_app, src_directory_name = source.split(':')
        
        # Get configurations
        src = config[src_app]

        src_directory = src["files"][src_directory_name]
        zip_filename = f"{src_directory['name']}.zip"
        ...
        print("🔌 Connecting to source server...")
        src_client = ssh_connect(src["ssh"]["ssh_key"], src["host"], src["ssh"]["port"], src["ssh"]["user"])
        file.create_backup(src_client, src_directory["user"], src_directory["password"], src_directory["name"])

        # Step 2: Download to local
        file.transfer_from_source(src_client, f"{src_directory['name']}.zip", f"./{zip_filename}")
        src_client.close()
